...
function tryNotifications(ms){if(!('Notification'in window)||Notification.permission!=='granted')return;const saved=JSON.parse(localStorage.getItem('golRadarNotified')||'{}');for(const m of ms){const fixture=m.fixture_id,score=`${m.home_goals||0}-${m.away_goals||0}`;if(m.bot_pick_best){const k=`BOT:${fixture}:${score}`;if(!saved[k]){new Notification(`🧠 BOT PICK • ${m.bot_pick_score||0}/100`,{body:`${m.home_team} ${score} ${m.away_team} • ${m.minute||0}' • ${m.bot_pick_text||''}${m.live_odd?` • Oran ${Number(m.live_odd).toFixed(2)}`:''}`});saved[k]=Date.now()}}const hs=Number(m.home_goal_signal||0),as=Number(m.away_goal_signal||0);if(hs>=65||as>=65){const side=hs>=as?'HOME':'AWAY',k=`RADAR:${fixture}:${side}:${score}`;if(!saved[k]){const team=side==='HOME'?m.home_team:m.away_team,sig=side==='HOME'?hs:as;new Notification(`⚽ GOL RADARI • %${sig}`,{body:`${m.home_team} ${score} ${m.away_team} • ${m.minute||0}' • Gol beklenen: ${team}`});saved[k]=Date.now()}}}localStorage.setItem('golRadarNotified',JSON.stringify(saved))}
